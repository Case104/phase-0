...
describe 'PERSON 3: my_array_sorting_method!' do
  ...
  it "converts sorts all elements alphabetically without altering changing data" do
    expect(my_array_sorting_method(i_want_pets)).to eq [2, 3, "but", "I", "have", "only", "pets", "want"]
  end

#   it "operates non-destructively" do
#      expect(my_array_sorting_method(i_want_pets).object_id).to_not eq(i_want_pets.object_id)me
#   end
